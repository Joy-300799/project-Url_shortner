...
const urlShort = async function (req, res) {
  ...
  try {
    ...
    // console.log(JSON.parse(checkInCache));

    if (checkInCache) {
      return res.status(200).send({
        status: true,
        message: `Response from cache.`,
        data: JSON.parse(checkInCache),
      });
    }
    ...
    await SET_ASYNC(`${shortCode}`, JSON.stringify(shortUrl));
    await SET_ASYNC(`${longUrl}`, JSON.stringify(shortUrl)); //setting in cache -> new entries
...
let redirectUrl = async function (req, res) {
  ...
  try {
    let cachedLongUrl = await GET_ASYNC(shortId);
